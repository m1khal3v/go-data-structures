...
func (list *SinglyLinkedList[T]) Push(index uint64, value T) bool {
	...
	default:
		...
		fromHead := list.size-1-index > index
		if fromHead {
			...
			for previous, currentIndex = list.tail.previous, list.size-2; currentIndex != previousIndex; previous, currentIndex = previous.previous, currentIndex-1 {
			}
		}
...
func (list *SinglyLinkedList[T]) Set(index uint64, value T) bool {
	...
	default:
		...
		previous.next.next.previous = newElement
		previous.next = newElement
	}
...
func (list *SinglyLinkedList[T]) Get(index uint64) (T, bool) {
	...
	default:
		...
		fromHead := list.size-1-index > index
		if fromHead {
			for current, currentIndex = list.head, uint64(0); currentIndex != index; current, currentIndex = current.next, currentIndex+1 {
			}
		} else {
			for current, currentIndex = list.tail.previous, list.size-2; currentIndex != index; current, currentIndex = current.previous, currentIndex-1 {
			}
		}
